...
impl WeatherHandler {
    ...
    pub fn get_info_from_weather_api(&self, city: &str) -> Result<WeatherData, Box<dyn std::error::Error>> {
         
        let url = format!("http://api.weatherapi.com/v1/current.json?key={}&q={}&aqi=no&lang=pl", self.key, city);
